...
Here are the data for `{python} len(report.sessions)` exercise sessions. I start monitoring as soon as I start running, the `{python} (_cdw[1] - _cdw[0]) // 60` minutes of cool down start at second `{python} standard_3x4.get_cooldown_window()[0]`.
...
At each second the model provides a (posterior) distribution for the heart rate or its acceleration. For example, this is the distribution of the heart rate for the exercise session `{python} ex_session.session_id` at two time points.
